namespace OrderManagement.Modules.Registrations.Infrastructure.Configuration.Processing.Outbox
{
    internal class ProcessOutboxCommandHandler : ICommandHandler<ProcessOutboxCommand>
    {
        ...
        public async Task Handle(ProcessOutboxCommand command, CancellationToken cancellationToken)
        {
            ...
            var schemaExists = await connection.ExecuteScalarAsync<int>("SELECT COUNT(*) FROM sys.schemas WHERE name = 'registrations'");
            Console.WriteLine(schemaExists > 0 ? "ProcessOutboxCommand registrations exists!" : "ProcessOutboxCommand registrations NOT found!");
            const string sql = $"""
                               SELECT 
